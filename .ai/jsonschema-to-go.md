# AI instructions: from JSON Schema to Go structs

Generate inside `pkg/kickr/v1` in the file `kickr.go` the Golang structures according to the local JSON Schema `.schemas/kickr.v1.schema.yml`.
...
- Structs' fields must be sorted exactly as ordered in the schema.
