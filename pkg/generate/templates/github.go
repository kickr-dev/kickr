...
func GitHub() []engine.Template[types.KickrWrapper] {
	return slices.Concat(githubWorkflow(), githubConfig())
}

func githubWorkflow() []engine.Template[types.KickrWrapper] {
	var templates []engine.Template[types.KickrWrapper]
...
func githubConfig() []engine.Template[types.KickrWrapper] {
	var templates []engine.Template[types.KickrWrapper]
	...
	templates = append(templates, engine.Template[types.KickrWrapper]{
		...
		Out:        release,
		Remove:     func(config types.KickrWrapper) bool { return config.Platform != parser.GitHub },
	})
